...
// `chatMessageRowsLoad` driven by an `IN (...)` query.
export async function chatListByScope(
    scope: 'public' | 'admin',
    ...
    try {
        ...
        for (const chat of rows) {
            const messageRows = await chatMessageRowsLoad(serverRuntime.db, chat.chatId);
