...
// to one trip.
export async function tripGet(tripId: string, requestingSession: GqlSSession, serverRuntime: ServerRuntime): Promise<GqlSTrip | null> {
    try {
        ...
        const days = dayRows.map((d) => toGqlTripDay(d, activitiesByDayId.get(d.tripDayId) ?? []));
        const packingItems = packingRows.map(toGqlTripPackingItem);
        return toGqlTrip(row, days, packingItems);
    } catch (error) {
