...
import type { FileUpload, MedicalAppointment, MedicalCategory, MedicalRecord, MedicalRecordFile } from '../db/schema';
import type { ServerRuntime } from '../domain/ServerRuntime';
import type { GqlSMedicalCategoryOverview, GqlSSession } from '../graphql/generated';
...
// One row per `MedicalCategory` for the Overview tab. For each category:
...
export async function medicalCategoryOverview(
    requestingSession: GqlSSession,
    serverRuntime: ServerRuntime,
): Promise<GqlSMedicalCategoryOverview[]> {
    try {
        const now = new Date();
        ...
        const appointmentsByCategory = new Map<MedicalCategory, MedicalAppointment[]>();
        ...
        const recordsByCategory = new Map<MedicalCategory, MedicalRecord[]>();
        ...
        const shortlistedRecords: MedicalRecord[] = [];
        const shortlistedByCategory = new Map<MedicalCategory, MedicalRecord[]>();
        ...
        return medicalCategories.map((category) => {
            ...
            return {
                ...
                nextDueAt,
                isOverdue,
                upcoming: upcoming.map(toGqlMedicalAppointment),
                recentRecords,
            } satisfies GqlSMedicalCategoryOverview;
        });
    } catch (error) {
