...
function buildSystemPrompt(snapshot: string): string {
    return [
        ...
        'unambiguously asked you to file, edit, or delete something.',
        '',
        currentDateForAgent(),
        '',
        ...
        '- `medicalOverview` — per-category cadence / last-visit / next-due / overdue view. Use for',
        '  "am I overdue for anything?", "when was my last GP visit?", "what\'s coming up medically?".',
        '- `medicalAppointmentsList`, `medicalRecordsList` — read full rows when the snapshot below is not enough.',
        '- `medicalRecordUpsert` — primary write. Files a health-journal record. Set `fileUploadIds` when Cem',
        '  included photos in this turn (the orchestrator forwards them to you via the brief).',
        '- `medicalRecordDelete` — remove a record (attachments removed with it).',
        '- `medicalAppointmentUpsert` — schedule a new visit, edit an existing one, or backdate a completed visit',
        '  that was never in the system.',
        '- `medicalAppointmentComplete` — shortcut for "I just went to the dentist" — flips status to `completed`,',
        '  stamps `completedAt`, optionally writes `nextDueAt` for the follow-up. Prefer over `medicalAppointmentUpsert`.',
        '- `medicalAppointmentDelete` — permanent delete. Prefer editing to `cancelled` or `missed` unless the user',
        '  really wants the row gone.',
        '- `medicalRecordFileAttach` — attach a file to an already-existing record. For the initial write, prefer',
        '  bundling `fileUploadIds` on `medicalRecordUpsert`.',
        '',
        'Rules:',
