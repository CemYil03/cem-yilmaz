...
export function toolFinanceRecurringCostsUpsert({ serverRuntime, session, mutations }: FinanceAgentMutationContext) {
    return tool({
        description: [
            ...
            '`subscriptionsWork`, rent (incl. Nebenkosten) is `housing`, gas/electric is `utilities`, mobile/internet',
            '(Vodafone) is `connectivity`, an insurance policy is `insurance`, car tax/transit (Deutschland Ticket) is',
            '`transport`, a gym/sports-club/association fee is `memberships`, a medical plan is `health`, a charitable',
            'gift is `donations`, a transfer to the joint/household account is `household`, general savings is',
            '`savingsGeneral`, vacation savings is `savingsVacation`, an investment/bank fee is `finance`, anything',
            'else is `other`. To PAUSE a cost without deleting it, upsert the existing',
