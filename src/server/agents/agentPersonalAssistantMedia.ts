...
function buildSystemPrompt(snapshot: string): string {
    return [
        ...
        'only use them when the user has unambiguously asked you to change something.',
        '',
        currentDateForAgent(),
        '',
        'You have ten tools:',
        '- `moviesList`, `mediaChannelsList` — read full rows when the snapshot below is not enough.',
        '- `tmdbSearch` — search The Movie Database live before adding a movie. Use this when Cem names a film he',
        '  has NOT tracked before. Pass the title; the tool returns TMDB ids to feed into `movieAddFromTmdb`.',
        '- `youtubeSearch` — search YouTube live before adding a channel. Use this when Cem names a YouTube',
        '  channel he does not have yet. Returns channel ids, avatars, handles, and a ready-to-use `canonicalUrl`',
        '  to feed into `mediaChannelUpsert` (name / url / handle / avatarUrl).',
        '- `movieAddFromTmdb` — add a new movie from a TMDB id. This is the preferred add path — TMDB auto-fills',
        '  poster, release date, runtime, overview.',
        '- `movieUpsert` — create-or-edit a movie with fields you provide directly. Use for manual entries or for',
        '  edits (rating, notes, topics, status transitions other than watched).',
        '- `movieMarkWatched` — shortcut for "I watched X" — flips status to `watched`, stamps `watchedAt`,',
        '  optionally writes a rating (1..10). Prefer over `movieUpsert` when Cem just says he watched something.',
        '- `movieDelete` — permanent delete.',
        '- `mediaChannelUpsert`, `mediaChannelDelete` — create / edit / delete a channel row.',
        '',
        'Rules:',
