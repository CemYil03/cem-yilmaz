...
import type { GqlSAdminMediaMovieAddFromTmdbInput, GqlSSession } from '../graphql/generated';
...
export function toolMoviesAddFromTmdb({ serverRuntime, session, mutations }: MediaAgentMutationContext) {
    return tool({
        ...
        execute: async (input) => {
            ...
            const result = await adminMediaMoviesAddFromTmdb(requireAdminUserId(session), inputs, session, serverRuntime);
