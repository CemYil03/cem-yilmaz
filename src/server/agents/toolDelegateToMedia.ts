...
import type { MediaAgentMutation, MediaAgentMutationLog } from './agentPersonalAssistantMedia';
...
export function toolDelegateToMedia({ serverRuntime, session, chatId, generationId, preWrittenToolCallIds }: DelegateToMediaContext) {
    return tool({
        description: [
            ...
            'retry automatically and do NOT confabulate softer phrasings.',
        ].join(' '),
        ...
        execute: async (input, { toolCallId }) => {
            ...
            const mutations: MediaAgentMutationLog = [];
            ...
            let toolResult:
                | { status: 'completed'; summary: string; mutations: MediaAgentMutation[] }
                | { status: 'needsMoreInfo' | 'noOp'; summary: string; missingFields: string[]; mutations: MediaAgentMutation[] }
                | { status: 'failed'; summary: string; mutations: MediaAgentMutation[] };
            try {
