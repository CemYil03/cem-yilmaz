...
import type { GqlSAdminProjectActivityCreate, GqlSSession } from '../graphql/generated';
...
export function toolProjectActivitiesUpsert({ serverRuntime, session, mutations }: ProjectsAgentMutationContext) {
    return tool({
        ...
        execute: async (input) => {
            ...
            const result = await adminProjectActivitiesUpsert(requireAdminUserId(session), inputs, session, serverRuntime);
