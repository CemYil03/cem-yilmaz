...
import type { GqlSMutationResult, GqlSSession, GqlSShowAddFromTmdbInput } from '../graphql/generated';
...
export async function showsAddFromTmdb(
    userId: string,
    inputs: readonly GqlSShowAddFromTmdbInput[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            for (let index = 0; index < inputs.length; index += 1) {
                ...
                if (existing) {
                    ...
                    if (!updated) {
                        throw new Error(`showsAddFromTmdb: refresh failed for tmdbId ${input.tmdbId}`);
                    }
                ...
                if (!inserted) {
                    throw new Error('showsAddFromTmdb: insert returned no rows');
                }
