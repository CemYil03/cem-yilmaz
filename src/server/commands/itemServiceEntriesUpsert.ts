...
import type { GqlSItemServiceEntryInput, GqlSMutationResult, GqlSSession } from '../graphql/generated';
...
// read.
export async function itemServiceEntriesUpsert(
    userId: string,
    inputs: readonly GqlSItemServiceEntryInput[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`itemServiceEntriesUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
