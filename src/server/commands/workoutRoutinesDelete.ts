...
// `WorkoutSession.routineId` pointing here is nulled (the session survives as
// an ad-hoc log).
export async function workoutRoutinesDelete(
    userId: string,
    ...
    try {
        ...
        if (deleted.length !== routineIds.length) {
            ...
            throw new Error(`workoutRoutinesDelete: rows not found: ${missing.join(', ')}`);
        }
