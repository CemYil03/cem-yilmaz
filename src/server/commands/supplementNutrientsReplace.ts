...
import type { SupplementNutrientCreate } from '../db/schema';
import type { ServerRuntime } from '../domain/ServerRuntime';
import type { GqlSMutationResult, GqlSSupplementNutrientInput, GqlSSession } from '../graphql/generated';
...
// transaction. Kept separate from `supplementsUpsert` because the codegen's
...
export async function supplementNutrientsReplace(
    userId: string,
    supplementId: string,
    nutrientInputs: readonly GqlSSupplementNutrientInput[],
    requestingSession: GqlSSession,
    serverRuntime: ServerRuntime,
): Promise<GqlSMutationResult> {
    const now = new Date();
    const payloads: SupplementNutrientCreate[] = nutrientInputs.map((nutrient, index) => ({
    ...
    try {
        await serverRuntime.db.transaction(async (transaction) => {
            ...
            if (!parent) {
                throw new Error(`supplementNutrientsReplace: supplement not found: ${supplementId}`);
            }
