...
import type { GqlSAdminNutritionMealPlanEntryInput, GqlSAdminNutritionRecipeInput } from '../graphql/generated';
import { commandSetup, testDb } from '../test/commandTestUtils';
import { adminNutritionMealPlanEntriesUpsert } from './adminNutritionMealPlanEntriesUpsert';
import { adminNutritionRecipesUpsert } from './adminNutritionRecipesUpsert';

const baseInput = (overrides: Partial<GqlSAdminNutritionMealPlanEntryInput> = {}): GqlSAdminNutritionMealPlanEntryInput => ({
    entryId: null,
    date: '2026-07-13',
    mealType: 'dinner',
    recipeId: null,
...
describe('adminNutritionMealPlanEntriesUpsert', () => {
    ...
    it('links a slot to an existing recipe', async () => {
        const { serverRuntime, requestingSession } = await commandSetup();
        const recipeInput: GqlSAdminNutritionRecipeInput = {
            recipeId: null,
            title: 'Chili',
            mealType: 'dinner',
            ingredients: [],
        ...
        const seededRecipe = await adminNutritionRecipesUpsert(requestingSession.userId!, [recipeInput], requestingSession, serverRuntime);
    ...
    it('throws when the referenced recipe does not exist', async () => {
        ...
        await expect(
            adminNutritionMealPlanEntriesUpsert(
                requestingSession.userId!,
                [baseInput({ recipeId: missingRecipeId })],
                requestingSession,
                serverRuntime,
            ),
