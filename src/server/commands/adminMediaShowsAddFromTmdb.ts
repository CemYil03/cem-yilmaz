...
export function toolShowsAddFromTmdb({ serverRuntime, session }: MediaAgentToolContext) {
    return tool({
        description: [
        ...
        execute: async (input) => {
            ...
            return adminMediaShowsAddFromTmdb(requireAdminUserId(session), inputs, session, serverRuntime);
        },
    });
}
