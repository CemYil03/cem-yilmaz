...
import type { GqlSExerciseInput, GqlSMutationResult, GqlSSession } from '../graphql/generated';
import type { ServerRuntime } from '../domain/ServerRuntime';

// Batch upsert of catalog exercises. Every row with an `exerciseId` is
// updated; every row without one is inserted.
export async function exercisesUpsert(
    userId: string,
    inputs: readonly GqlSExerciseInput[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`exercisesUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
