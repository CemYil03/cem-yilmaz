...
import type { GqlSMutationResult, GqlSProjectFileUpsert, GqlSSession } from '../graphql/generated';
...
// row (in input order).
export async function projectFilesUpsert(
    userId: string,
    inputs: readonly GqlSProjectFileUpsert[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`projectFilesUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
            ...
            if (insertUploadIds.length > 0) {
                ...
                if (uploads.length !== insertUploadIds.length) {
                    ...
                    throw new Error(`projectFilesUpsert: fileUpload(s) not found: ${missing.join(', ')}`);
                }
            }
