...
import type { ProjectActivityCreate, ProjectActivityDirection, ProjectActivityKind } from '../db/schema';
import type { ServerRuntime } from '../domain/ServerRuntime';
import type { GqlSMutationResult, GqlSProjectActivityCreate, GqlSSession } from '../graphql/generated';
...
// sessions are owned by `projectTimersStart` / `projectTimersStop`, so any
...
// on create: when present the command inserts a matching `ProjectLink` /
// `ProjectFile` in the same transaction with `activityId` pointing at the new
...
export async function projectActivitiesUpsert(
    userId: string,
    inputs: readonly GqlSProjectActivityCreate[],
    ...
    const seeds = inputs.map((input) => {
        if (input.kind === 'work') {
            throw new Error('projectActivitiesUpsert: work-kind rows are owned by the timer mutations');
        }
        if (input.channel && input.kind !== 'clientContact' && input.kind !== 'meeting') {
            throw new Error(`projectActivitiesUpsert: channel is only valid for clientContact / meeting (got kind='${input.kind}')`);
        }
        if ((input.amountCents != null || input.offerStatus != null) && input.kind !== 'offer') {
            throw new Error(`projectActivitiesUpsert: amountCents / offerStatus are only valid for offer (got kind='${input.kind}')`);
        }
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`projectActivitiesUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
            ...
            for (const { activityId, input, isUpdate, payload } of seeds) {
                ...
                if (hasAttachFile) {
                    const [upload] = await transaction
                        ...
                        .limit(1);
                    if (!upload) throw new Error(`projectActivitiesUpsert: fileUpload ${input.attachFileUploadId} not found`);
