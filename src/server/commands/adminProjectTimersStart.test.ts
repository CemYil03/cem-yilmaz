...
import { adminProjectTimersStart } from './adminProjectTimersStart';
import { adminProjectTimersStop } from './adminProjectTimersStop';

const projectSeed = async (overrides: Partial<AdminProjectCreate> = {}) => {
...
describe('adminProjectTimersStart', () => {
    it('inserts a running timer row when no other timer is active', async () => {
        ...
        // Act
        const result = await adminProjectTimersStart(
    ...
    it('stops the existing running timer before starting a new one', async () => {
        ...
        const startA = await adminProjectTimersStart(
            requestingSession.userId!,
            [{ projectId: projectA.projectId, taskId: null, title: 'first' }],
            requestingSession,
            serverRuntime,
        );
        const startB = await adminProjectTimersStart(
            requestingSession.userId!,
    ...
    it('adminProjectTimersStop stamps endedAt and computes durationSec', async () => {
        // Arrange
        const { serverRuntime, requestingSession } = await commandSetup();
        const project = await projectSeed();
        const started = await adminProjectTimersStart(
            requestingSession.userId!,
            [{ projectId: project.projectId, taskId: null, title: null }],
            requestingSession,
            serverRuntime,
        ...
        // Act
        await adminProjectTimersStop(requestingSession.userId!, [activityId], requestingSession, serverRuntime);
    ...
    it('adminProjectTimersStop is idempotent on an already-stopped row', async () => {
        // Arrange
        const { serverRuntime, requestingSession } = await commandSetup();
        const project = await projectSeed();
        const started = await adminProjectTimersStart(
    ...
    it('adminProjectTimersStop throws if an id does not exist', async () => {
        // Arrange
        const { serverRuntime, requestingSession } = await commandSetup();

        // Act + Assert
        await expect(
            adminProjectTimersStop(requestingSession.userId!, [crypto.randomUUID()], requestingSession, serverRuntime),
