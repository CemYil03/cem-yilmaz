...
import type { GqlSItemFileAttachInput, GqlSMutationResult, GqlSSession } from '../graphql/generated';
...
// `medicalRecordFilesAttach` — the client first uploads bytes via
...
// order.
export async function itemFilesAttach(
    userId: string,
    inputs: readonly GqlSItemFileAttachInput[],
    ...
    try {
        ...
        if (foreign.length > 0) {
            throw new Error(`itemFilesAttach: fileUpload(s) not owned by user ${userId}: ${foreign.join(', ')}`);
        }
