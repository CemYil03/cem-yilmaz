...
import type { GqlSAdminMutationTripPackingItemToggleArgs, GqlSSession, GqlSTripPackingItem } from '../graphql/generated';
...
export async function tripPackingItemToggle(
    ...
    try {
        const [existing] = await serverRuntime.db
            ...
            .limit(1);
        if (!existing) throw new Error(`tripPackingItemToggle: row ${args.tripPackingItemId} not found`);
