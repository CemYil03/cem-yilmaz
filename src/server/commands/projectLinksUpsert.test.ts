...
import { projectLinksDelete } from './projectLinksDelete';
import { projectLinksUpsert } from './projectLinksUpsert';
...
describe('projectLinksUpsert', () => {
    it('creates a link with the given kind and pin state', async () => {
        ...
        const result = await projectLinksUpsert(
    ...
    it('updates label / kind / pinned without changing activityId', async () => {
        ...
        const projectId = await projectSeed();
        const activityId = await activitySeed(projectId);
        const created = await projectLinksUpsert(
            requestingSession.userId!,
            [
                {
                    projectLinkId: null,
...
describe('projectLinksDelete', () => {
    it('removes the row and returns success', async () => {
        ...
        const result = await projectLinksDelete(requestingSession.userId!, [linkId], requestingSession, serverRuntime);
