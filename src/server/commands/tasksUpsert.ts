...
import type { GqlSMutationResult, GqlSSession, GqlSTaskCreate } from '../graphql/generated';
...
export async function tasksUpsert(
    userId: string,
    inputs: readonly GqlSTaskCreate[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`tasksUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
