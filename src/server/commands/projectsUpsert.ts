...
import type { GqlSMutationResult, GqlSProjectCreate, GqlSSession } from '../graphql/generated';
...
// 2. Conversion from a `ProjectRequest` — on a create with `sourceRequestId`
...
export async function projectsUpsert(
    userId: string,
    inputs: readonly GqlSProjectCreate[],
    ...
    const seeds = inputs.map((input) => {
        ...
        if (isUpdate && (input.position === null || input.position === undefined)) {
            throw new Error('projectsUpsert: position is required when updating an existing project');
        }
        return {
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`projectsUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
            ...
            for (const { projectId, input, isUpdate } of seeds) {
                ...
                if (!isUpdate && input.sourceRequestId) {
                    ...
                    if (!row) {
                        throw new Error(`projectsUpsert: source request ${input.sourceRequestId} not found`);
                    }
                    if (row.status !== 'emailVerified') {
                        throw new Error(
                            `projectsUpsert: source request ${input.sourceRequestId} is in state '${row.status}', expected 'emailVerified'`,
                        );
                    }
                    sourceRequest = row;
                }
