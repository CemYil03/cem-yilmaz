...
import type { GqlSFinanceRecurringCostInput } from '../graphql/generated';
import { commandSetup, testDb } from '../test/commandTestUtils';
import { financeRecurringCostsUpsert } from './financeRecurringCostsUpsert';
...
describe('financeRecurringCostsUpsert', () => {
    it('inserts a new row when no costId is supplied', async () => {
        // Arrange
        const { serverRuntime, requestingSession } = await commandSetup();

        // Act
        const result = await financeRecurringCostsUpsert(requestingSession.userId!, [baseInput()], requestingSession, serverRuntime);
    ...
    it('updates the existing row when costId is supplied', async () => {
        ...
        const { serverRuntime, requestingSession } = await commandSetup();
        const seeded = await financeRecurringCostsUpsert(requestingSession.userId!, [baseInput()], requestingSession, serverRuntime);
        const createdId = seeded.referenceIds![0]!;

        // Act
        const updated = await financeRecurringCostsUpsert(
    ...
    it('throws when updating a row that does not exist', async () => {
        ...
        // Act + Assert
        await expect(
            financeRecurringCostsUpsert(requestingSession.userId!, [baseInput({ costId: missingId })], requestingSession, serverRuntime),
    ...
    it('echoes referenceIds in input order for a mixed insert + update batch', async () => {
        ...
        const { serverRuntime, requestingSession } = await commandSetup();
        const seedA = await financeRecurringCostsUpsert(
            requestingSession.userId!,
            [baseInput({ name: 'Seed A' })],
            requestingSession,
            serverRuntime,
        );
        const seedB = await financeRecurringCostsUpsert(
            requestingSession.userId!,
        ...
        const result = await financeRecurringCostsUpsert(
            requestingSession.userId!,
            [
