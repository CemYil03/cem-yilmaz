...
// preserved (same posture as `projectFilesDelete`) — the upload belongs to the
...
export async function itemFilesDelete(
    userId: string,
    ...
    try {
        ...
        if (deleted.length !== itemFileIds.length) {
            ...
            throw new Error(`itemFilesDelete: rows not found: ${missing.join(', ')}`);
        }
