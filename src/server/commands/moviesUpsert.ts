...
import type { GqlSMovieInput, GqlSMutationResult, GqlSSession } from '../graphql/generated';
...
// re-insert of the same TMDB movie would fail here — `moviesAddFromTmdb`
...
export async function moviesUpsert(
    userId: string,
    inputs: readonly GqlSMovieInput[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`moviesUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
