...
import type { GqlSMedicalRecordFileAttachInput, GqlSMutationResult, GqlSSession } from '../graphql/generated';

// Batch attach of already-uploaded `FileUploads` rows to medical records.
// Same two-step upload contract as `itemFilesAttach` — the client first
...
export async function medicalRecordFilesAttach(
    userId: string,
    inputs: readonly GqlSMedicalRecordFileAttachInput[],
    ...
    try {
        ...
        if (foreign.length > 0) {
            throw new Error(`medicalRecordFilesAttach: fileUpload(s) not owned by user ${userId}: ${foreign.join(', ')}`);
        }
