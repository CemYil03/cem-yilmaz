...
import type { WorkoutRoutineItemCreate } from '../db/schema';
import type { GqlSMutationResult, GqlSSession, GqlSWorkoutRoutineItemInput } from '../graphql/generated';
...
export async function workoutRoutineItemsUpsert(
    userId: string,
    inputs: readonly GqlSWorkoutRoutineItemInput[],
    ...
    try {
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            ...
            if (routines.length !== parentRoutineIds.length) {
                ...
                throw new Error(`workoutRoutineItemsUpsert: parent routines not found: ${missing.join(', ')}`);
            }
            ...
            if (foundExercises.length !== exerciseIds.length) {
                ...
                throw new Error(`workoutRoutineItemsUpsert: exercises not found: ${missing.join(', ')}`);
            }
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`workoutRoutineItemsUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
            ...
            for (const { routineItemId, item, isUpdate } of seeds) {
                ...
                const payload: WorkoutRoutineItemCreate = {
                    routineItemId,
