...
import type { GqlSMutationResult, GqlSProjectTimerStartInput, GqlSSession } from '../graphql/generated';
...
//
// Kept separate from `projectActivitiesUpsert` because the server-side clock
// semantics (stop-open-timer + `startedAt: now`) are distinct from a plain
// field-set upsert.
export async function projectTimersStart(
    userId: string,
    inputs: readonly GqlSProjectTimerStartInput[],
