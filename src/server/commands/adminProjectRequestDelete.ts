...
import type { GqlSAdminMutationAdminProjectRequestDeleteArgs, GqlSMutationResult, GqlSSession } from '../graphql/generated';
...
// verification; use `adminProjectRequestArchive` for the routine "no thanks" case.
export async function adminProjectRequestDelete(
    userId: string,
    ...
    try {
        ...
        if (deleted.length === 0) {
            throw new Error(`adminProjectRequestDelete: row ${args.projectRequestId} not found`);
        }
