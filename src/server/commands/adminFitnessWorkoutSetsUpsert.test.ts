...
import type {
    GqlSAdminFitnessExerciseInput,
    GqlSAdminFitnessWorkoutSessionInput,
    GqlSAdminFitnessWorkoutSetInput,
} from '../graphql/generated';
import { commandSetup, testDb } from '../test/commandTestUtils';
import { adminFitnessExercisesUpsert } from './adminFitnessExercisesUpsert';
import { adminFitnessWorkoutSessionsUpsert } from './adminFitnessWorkoutSessionsUpsert';
import { adminFitnessWorkoutSetsUpsert } from './adminFitnessWorkoutSetsUpsert';
...
async function seedParents() {
    ...
    const exerciseInput: GqlSAdminFitnessExerciseInput = {
        exerciseId: null,
        name: 'Back Squat',
        muscleGroup: 'legs',
        equipment: 'barbell',
    ...
    const exercise = await adminFitnessExercisesUpsert(userId, [exerciseInput], requestingSession, serverRuntime);
    const exerciseId = exercise.referenceIds![0]!;

    const sessionInput: GqlSAdminFitnessWorkoutSessionInput = {
        sessionId: null,
        date: '2026-07-09',
        title: 'Leg day',
        routineId: null,
    ...
    const workout = await adminFitnessWorkoutSessionsUpsert(userId, [sessionInput], requestingSession, serverRuntime);
...
const setInput = (
    sessionId: string,
    exerciseId: string,
    overrides: Partial<GqlSAdminFitnessWorkoutSetInput> = {},
): GqlSAdminFitnessWorkoutSetInput => ({
    setId: null,
    sessionId,
    exerciseId,
...
describe('adminFitnessWorkoutSetsUpsert', () => {
    it('logs a 5×5 as five rows against one session in input order', async () => {
        ...
        const result = await adminFitnessWorkoutSetsUpsert(userId, inputs, requestingSession, serverRuntime);
    ...
    it('throws when the parent session does not exist', async () => {
        ...
        await expect(
            adminFitnessWorkoutSetsUpsert(userId, [setInput(missingSessionId, exerciseId)], requestingSession, serverRuntime),
        ).rejects.toThrow(/parent sessions not found/);
    });
    ...
    it('throws when the exercise does not exist', async () => {
        ...
        await expect(
            adminFitnessWorkoutSetsUpsert(userId, [setInput(sessionId, missingExerciseId)], requestingSession, serverRuntime),
        ).rejects.toThrow(/exercises not found/);
    });

    it('updates an existing set in place', async () => {
        const { serverRuntime, requestingSession, userId, exerciseId, sessionId } = await seedParents();
        const seeded = await adminFitnessWorkoutSetsUpsert(userId, [setInput(sessionId, exerciseId)], requestingSession, serverRuntime);
        const setId = seeded.referenceIds![0]!;

        await adminFitnessWorkoutSetsUpsert(
            userId,
