...
import { cvExperiencesDelete } from './cvExperiencesDelete';
...
describe('cvExperiencesDelete', () => {
    it('removes the row and reports success', async () => {
        ...
        // Act
        const result = await cvExperiencesDelete(requestingSession.userId!, [createdId], requestingSession, serverRuntime);
    ...
    it('throws when the row does not exist', async () => {
        ...
        // Act + Assert
        await expect(cvExperiencesDelete(requestingSession.userId!, [missingId], requestingSession, serverRuntime)).rejects.toThrow(
            /not found/,
        );
    ...
    it('deletes every id in a batch and echoes them in input order', async () => {
        ...
        const result = await cvExperiencesDelete(requestingSession.userId!, [idB, idC, idA], requestingSession, serverRuntime);

        // Assert
        expect(result.referenceIds).toEqual([idB, idC, idA]);
        for (const id of [idA, idB, idC]) {
            const rows = await testDb.select().from(cvExperience).where(eq(cvExperience.cvExperienceId, id));
            expect(rows).toHaveLength(0);
        }
    ...
    it('throws when any id in the batch does not exist', async () => {
        ...
        // Act + Assert
        await expect(cvExperiencesDelete(requestingSession.userId!, [idA, missingId], requestingSession, serverRuntime)).rejects.toThrow(
            /not found/,
        );
        expect(serverRuntime.publish.userUpdates).not.toHaveBeenCalled();
    });
