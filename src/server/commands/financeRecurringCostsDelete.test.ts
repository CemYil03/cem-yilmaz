...
import { financeRecurringCostsDelete } from './financeRecurringCostsDelete';
...
describe('financeRecurringCostsDelete', () => {
    it('removes the row and reports success', async () => {
        ...
        // Act
        const result = await financeRecurringCostsDelete(requestingSession.userId!, [createdId], requestingSession, serverRuntime);
    ...
    it('throws when the row does not exist', async () => {
        ...
        // Act + Assert
        await expect(financeRecurringCostsDelete(requestingSession.userId!, [missingId], requestingSession, serverRuntime)).rejects.toThrow(
            /not found/,
        );
    ...
    it('deletes every id in a batch and echoes them in input order', async () => {
        ...
        const result = await financeRecurringCostsDelete(requestingSession.userId!, [idB, idC, idA], requestingSession, serverRuntime);
    ...
    it('throws when any id in the batch does not exist', async () => {
        ...
        // Act + Assert
        await expect(
            financeRecurringCostsDelete(requestingSession.userId!, [idA, missingId], requestingSession, serverRuntime),
