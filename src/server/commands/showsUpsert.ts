...
import type { GqlSMutationResult, GqlSSession, GqlSShowInput } from '../graphql/generated';
...
export async function showsUpsert(
    userId: string,
    inputs: readonly GqlSShowInput[],
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            if (updateIds.length > 0) {
                ...
                if (existing.length !== updateIds.length) {
                    ...
                    throw new Error(`showsUpsert: rows not found: ${missing.join(', ')}`);
                }
            }
