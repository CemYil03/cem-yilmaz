...
import type { ItemValuationCreate } from '../db/schema';
import type { ServerRuntime } from '../domain/ServerRuntime';
import type { GqlSItemRepriceInput, GqlSMutationResult, GqlSSession } from '../graphql/generated';
...
// input order.
//
// Kept separate from `itemsUpsert` because a reprice is more than a field-set:
// it appends a journal row, so a naive upsert could silently overwrite the
// cache without a matching journal entry.
export async function itemsReprice(
    userId: string,
    inputs: readonly GqlSItemRepriceInput[],
    ...
    const rows = inputs.map((input) => {
        const valuation: ItemValuationCreate = {
    ...
    try {
        ...
        await serverRuntime.db.transaction(async (transaction) => {
            ...
            if (existing.length !== itemIds.length) {
                ...
                throw new Error(`itemsReprice: rows not found: ${missing.join(', ')}`);
            }
