...
| `animate-chat-button-pulse` | `src/styles.css` (`@keyframes`) | One-shot ring pulse (1.2 s, single breath) when an overlay closes and the trigger button needs to draw the eye — "your conversation went there." Driven by a monotonic `highlightSignal` counter on the chat providers; the consumer (`HeaderChatButton`, `WorkspaceAssistantLauncher`) toggles the class for the animation window. |
