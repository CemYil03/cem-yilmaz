...
| Thoughts disclosure         | `src/web/components/AssistantReasoning.tsx` | User expand/collapse of Gemini thought summaries — height (`grid-template-rows` 0fr↔1fr) + opacity + chevron rotate, 200 ms `ease-out`. Answers "where did that go / where is it coming from?" Live-driven open/close stays instant (no fight with stick-to-bottom). `motion-reduce:transition-none`.                                         |
