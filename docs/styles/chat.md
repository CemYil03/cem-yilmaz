...
| Anchor new turns                           | **flush at top of viewport** (`scrollAnchor` on the **user** row only) | A new send settles the user message at the top so the reply grows into the full room below. Once the reply fills the viewport, follow-output takes over again.                                                                                                |
| Previous-item peek on turn anchor          | **0 px** (`scrollPreviousItemPeek={0}`)                                | shadcn's MessageScroller default is 64 px of the previous item peeking above a new turn. We override to 0 so the prior reply leaves the viewport entirely — no leftover strip of the last answer above the new question. Set once in `ChatTranscriptShell`.   |
...
user message) composes with that — a send from the live edge parks the new turn flush at the top of the viewport (no previous-item peek);
streaming growth is then followed until the reader scrolls away.
...
**`scrollAnchor` only on user messages.** That is the turn boundary: a newly appended user row settles flush at the top of the viewport so
the reply can grow into the room below (`scrollPreviousItemPeek={0}` — no leftover strip of the prior reply). Streaming / assistant / tool
rows must not set `scrollAnchor` — their growth is followed by `autoScroll` while the reader is at the live edge. Do not hand-roll scroll;
route through `ChatTranscriptShell` (see [The shared primitives](#the-shared-primitives)).
...
| Addon row packing      | Left cluster (`addonStart`) is `min-w-0 flex-1 overflow-hidden`; attach + Send are `shrink-0`                               | Narrow sidebars must never push Send past the composer edge. Workspace selects truncate (`max-w-*`) instead of growing with label length.                                                                                         |
...
| `ChatTranscriptShell`                                                                                   | `src/web/components/base/chat-transcript-shell.tsx` | You need a chat scroll container. Pins `defaultScrollPosition="last-anchor"`, `scrollEdgeThreshold={64}`, `scrollPreviousItemPeek={0}`, `autoScroll`, and the jump-to-latest pill. Streaming `aria-live` lives on the streaming item itself. **Every transcript uses this.**                                                          |
...
- **`data-autoscrolling:scrollbar-none` (or any `scrollbar-width: none`) on the transcript viewport.** Collapses the stable gutter while
  streaming follow-output is engaged — bubbles jump horizontally. See
  [Scrollbar gutter](#scrollbar-gutter--reserved-never-over-the-bubbles).
