...
| `TMDB_API_KEY`                 | No       | [The Movie Database](https://www.themoviedb.org/) v3 API key. Powers the `/workspace/media` search-as-you-type add flow and the `moviesAddFromTmdb` auto-fill. Missing key degrades to empty search results (manual entry still works) — see [features/workspace-media.md](./features/workspace-media.md)                 |
