...
Underlined section tabs at the top: `Heute` · `Diese Woche` · `Alles` · `Blockiert` · `Erledigt`, with the focus-mode toggle button pushed
to the right edge of the same row. Each tab carries a Lucide icon and a tooltip that spells out its rule. The active tab is reflected in the
URL as `?view=today|week|waiting|done` (the default `all` is omitted so URLs stay clean), so reloads and shared links preserve the current
view. Clicks use `replace` navigation — the back button doesn't get polluted with every filter tap. Matching rules:
...
- **Erledigt** — the archive of completed rows. Carries a count badge on the tab. When active it replaces the main list with a `DoneList`
  grouped by day (`Heute` / `Gestern` / `<weekday>, dd.MM.yyyy`), rows sorted by `completedAt` desc. Hover reveals **restore** and
  **delete** affordances; the composer and empty-reward states are hidden on this view.
...
**Filter-aware default `whenBucket`.** If the user is looking at the `Heute`, `Diese Woche`, or `Blockiert` view tab and captures a plain
...
Press `f` (or click the focus icon at the right end of the filter row) with any open todo present to enter focus mode: a fixed overlay
centres the top todo — title, meta line (effort · when-bucket · due date, only the present bits), and the full `notes` body rendered as a
prose block with preserved line breaks — so a task with real detail is actually usable inside the overlay. Everything else is pushed behind
a blurred backdrop. Enter completes the todo and exits; `Esc` exits without completing. The page-level `n` / `f` bindings are suspended
while the overlay is up so `n` can't focus a composer that isn't mounted. The overlay scrolls vertically for tasks with long notes. Focus
mode is reflected in the URL as `?mode=focus` so a reload keeps the user in the same surface and the chat assistant can deep-link into it.
...
Everything the user is currently looking at is encoded in query params so reloads and shared links reproduce the exact view.

| Param   | Values                                   | Meaning                                                         |
| ------- | ---------------------------------------- | --------------------------------------------------------------- |
| `focus` | task id                                  | Deep-link target: scroll into view, flash the ring, then clear. |
| `view`  | `today` \| `week` \| `waiting` \| `done` | Active view tab. Default `all` is omitted from the URL.         |
...
Every icon-only affordance on the page — the ⚡🎯🧠 default-effort pickers, the focus toggle at the right end of the view-tab row, the
per-row snooze / edit / delete buttons, the ritual checkbox itself, and the per-row restore / delete buttons in the `Erledigt` archive — is
wrapped in a Radix `Tooltip`. The site's `TooltipProvider` is mounted globally at `__root.tsx`. The view tabs carry a tooltip too,
describing the rule each one applies. Delay is short (~0ms) so hover feels responsive.
...
`todo-row-complete`) in addition to the checkbox draw and the hand-drawn strike-through. All animations respect
...
- The state transition (row leaves the open list and enters the `Erledigt` archive) still happens. Only the animation timing is dropped.
