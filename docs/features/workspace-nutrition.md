...
- **Supplements** — a card grid (alphabetical) of tracked supplements. Each card shows name, brand, serving size, servings-per-container,
  ...
  button: it calls the `adminNutritionSupplementResearch` action (grounded web search), fills the serving fields and replaces the editable
  nutrient-row repeater with the found composition, and shows the research summary (or a "couldn't find it" note when `found:false`).
  Nothing persists until Save — the dialog then runs `adminNutritionSupplementsUpsert` (parent row) followed by
  `adminNutritionSupplementNutrientsReplace` (the composition). `researchedAt` is stamped when the composition came from research.
...
Three admin-only tables — `AdminNutritionRecipe`, `AdminNutritionMealPlanEntry`, `AdminNutritionFoodLogEntry` — sharing a
`AdminNutritionMealType` enum. Plan slots optionally reference a recipe (`ON DELETE SET NULL`) or carry free text. The diary distinguishes
food from drink with a `kind` column. All three lists are fetched whole and sliced to the visible week client-side, keeping the args-free
seed-and-subscribe posture of every other workspace page. The nutrition sub-agent is the primary chat write path; the page is the manual
...
`mealTypes = ['breakfast','lunch','dinner','snack','other']` (SDL `AdminNutritionMealType`); `foodLogKinds = ['food','drink']` (SDL
`AdminNutritionFoodLogKind`).

- **`AdminNutritionRecipe`** — `recipeId`, `title`, `mealType`, `ingredients text[]`, `steps`, `tags text[]`, `isFavorite`, `rating`,
  `prepTimeMinutes`, `servings`, `sourceUrl`, `notes`, `lastMadeAt timestamptz`, timestamps. Indexes `(mealType)`, `(isFavorite)`.
- **`AdminNutritionMealPlanEntry`** — `entryId`, `date`, `mealType`, `recipeId` FK → Recipes **ON DELETE SET NULL**, `customText`, `notes`,
  timestamps. Index `(date)`.
- **`AdminNutritionFoodLogEntry`** — `logId`, `consumedAt timestamptz`, `mealType`, `kind`, `description`, `recipeId` FK → Recipes **ON
  DELETE SET NULL**, `notes`, timestamps. Index `(consumedAt)`.
- **`AdminNutritionSupplement`** — `supplementId`, `name`, `brand`, `servingSize`, `servingsPerContainer`, `sourceUrl`, `notes`,
  `researchedAt timestamptz` (stamped on AI fill), timestamps. Index `(name)`.
- **`AdminNutritionSupplementNutrient`** — `nutrientId`, `supplementId` FK → Supplements **ON DELETE cascade** (children owned by the
  parent, unlike the recipe SET NULL links), `name`, `amount varchar` (text tolerates `"<1"`/`"trace"`), `unit`, `percentDailyValue`,
  `sortOrder`, timestamps. Index `(supplementId)`. Migration: `drizzle/0026_low_proteus.sql`.
...
rows ordered by `sortOrder`). Write mutations on `AdminMutation` (gated once by `guardAdminMutation`):
`adminNutritionRecipesUpsert`/`adminNutritionRecipesDelete`, `adminNutritionMealPlanEntriesUpsert`/`adminNutritionMealPlanEntriesDelete`,
`adminNutritionFoodLogEntriesUpsert`/`adminNutritionFoodLogEntriesDelete`,
`adminNutritionSupplementsUpsert`/`adminNutritionSupplementsDelete`/`adminNutritionSupplementNutrientsReplace`, plus the non-persisting AI
action `adminNutritionSupplementResearch`. Every entity mutation is a batch returning
`MutationResult { success, referenceId, referenceIds }`; `referenceIds` echoes ids in input order. The page reconciles over `userUpdates`.
...
(`[AdminNutritionSupplementNutrientInput!]` inside `AdminNutritionSupplementInput`) without a `z.lazy()` wrapper that breaks its own
`Properties<T>` return type. So the parent and children are written by sibling batch mutations — `adminNutritionSupplementsUpsert` (parent
row) + `adminNutritionSupplementNutrientsReplace(supplementId, nutrients)` (delete-then-insert the whole child set with `sortOrder` = array
index) — the same parent/child split travel and inventory use.

**`adminNutritionSupplementResearch`** is a mutation, not a query: it is an explicit button-triggered AI action with external cost (grounded
search), not a cacheable read — the same posture as `adminMediaMoviesAddFromTmdb`. It returns a
`AdminNutritionSupplementResearchResult { found, brand, servingSize, servingsPerContainer, sourceUrl, notes, nutrients[], summary }` and
persists nothing.
...
`adminNutritionSupplementsUpsert` writes the parent row; `adminNutritionSupplementNutrientsReplace` deletes then re-inserts the child set in
one transaction; `adminNutritionSupplementsDelete` relies on the FK cascade. The `adminNutritionSupplementResearch` action is backed by
...
referenced recipes / nutrient rows in one pass. Mappers: `toGqlAdminNutritionRecipe`, `toGqlAdminNutritionMealPlanEntry`,
`toGqlAdminNutritionFoodLogEntry`, `toGqlAdminNutritionSupplement`, `toGqlAdminNutritionSupplementNutrient`. Resolver wiring in
`resolversCreate.ts`.
...
- **`agentPersonalAssistantNutrition.ts`** — `ToolLoopAgent` factory, model `ADMIN_CHAT_MODEL_FALLBACK_ID`, `stopWhen: [isStepCount(10)]`.
  ...
  X" → `adminNutritionFoodLogEntriesUpsert` rule, the "plan Tuesday dinner" → `adminNutritionMealPlanEntriesUpsert` rule, the "I made X" →
...
- **Tools**: reads `recipesList`, `mealPlanList`, `foodLogList`, `supplementsList`, `adminNutritionSupplementResearch` (grounded,
  non-persisting); writes `adminNutritionRecipesUpsert` (hand-built Zod — `lastMadeAt` is `DateTime`), `adminNutritionRecipesDelete`,
  `adminNutritionMealPlanEntriesUpsert` (generated schema — `Date` is safe), `adminNutritionMealPlanEntriesDelete`,
...
- **Recipe photos** — a `RecipeFiles` → `FileUploads` join (mirrors `AdminMedicalRecordFile`).
