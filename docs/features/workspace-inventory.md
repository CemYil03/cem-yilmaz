...
- **Item detail** (`/workspace/inventory/{itemId}`) is the busy surface:
  ...
  - **Files**: receipts, warranty PDFs, invoices, photos, manuals. Upload via the existing `POST /api/file-uploads` →
    `adminInventoryItemFilesAttach` two-step; each file carries a `kind` and can be pinned (the pin toggle goes through
    `adminInventoryItemFilesUpsert`). Files can be attached at the item level or to a specific service entry (invoices, typically) — the
    entry card shows the invoice inline, and the item-level Files section shows the same file too.
- **Disposal**: setting an item to sold / gifted / lost / disposed keeps the row (so material net worth is reconcilable and the history
  ...
  a field-set on `adminInventoryItemsUpsert` — the "Change status" action carries the existing row with the disposal fields overridden.
...
`itemCategories` is a `const` array in `src/server/db/schema.ts` mirrored as `AdminInventoryItemCategory` in the GraphQL schema. The
alternative — a user-editable `ItemCategories` table — was rejected for v1: the taxonomy is stable ("electronics", "appliance", "kitchen",
…), the UI autocomplete never needs to grow one-off values, and a new category is a one-line change plus a codegen run. Elevate to a table
only if the list starts churning weekly.

### Why a separate `AdminInventoryItemFile` join table

The alternative — a `fileUploadId` array on the item row — was rejected. `AdminInventoryItemFile` carries its own metadata (kind, pin,
optional service-entry link) and the same pattern already works on `AdminProjectFile`. Reusing `FileUploads` for the raw bytes means the
upload flow, the `/api/file-uploads/:id` download route, and the byte-storage strategy stay identical to chat attachments and project files.
Cascade semantics: deleting an item removes the join rows but preserves the underlying uploads (they belong to the user); deleting a service
entry sets the join's `serviceEntryId` to null so the invoice stays visible on the item.

### Why cache `currentValueCents` on the item

Every list read and the material-net-worth aggregate could compute the "latest by valuedAt" from `AdminInventoryItemValuation` — but at the
cost of a correlated subquery per row. Caching the latest valuation on the item lets the list surface hit one indexed table and read the
cached column directly. The `adminInventoryItemsReprice` command is the single writer: it inserts the journal row and updates the cache in
one transaction, so they can't drift.

### Why `adminInventoryItemsReprice` is a separate mutation (and disposal is not)

Repricing has a coupled side-effect: it appends an `AdminInventoryItemValuation` journal row **and** updates the cached `currentValueCents`
in one transaction. Folding it into `adminInventoryItemsUpsert` would let a naive edit silently overwrite the cache without a matching
journal row, so it stays a separate batch mutation (`adminInventoryItemsReprice`) that owns `currentValueCents`.

Disposal, by contrast, is a pure field-set (`disposalState` + `disposedAt`), so it **collapsed into `adminInventoryItemsUpsert`** — there is
no separate `itemDispose` mutation. The upsert applies the rule `disposedAt = 'owned' ? null : (disposedAt ?? now)`, so entering a disposal
state stamps the date and reverting to `owned` clears it. The edit dialog and the "Change status" action both go through
`adminInventoryItemsUpsert` carrying the existing row (from the subscription payload) with the disposal fields set, so a plain field edit
...
- `AdminInventoryItem` — one row per possession, plus a cached `currentValueCents` snapshot and a `disposalState` (owned / sold / gifted /
  lost / disposed).
- `AdminInventoryItemValuation` — repricing journal, `(itemId, valuedAt DESC)` indexed.
- `AdminInventoryItemServiceEntry` — service events with optional `nextDueAt`.
- `AdminInventoryItemFile` — join between items and `FileUploads`, with optional `serviceEntryId` for invoices tied to specific events.
...
- Write: 8 batch mutations under `extend type AdminMutation`, all returning `MutationResult { success, referenceId, referenceIds }` — the
  ...
  edits pass a one-element array): `adminInventoryItemsUpsert`, `adminInventoryItemsDelete`, `adminInventoryItemsReprice`,
  `adminInventoryItemServiceEntriesUpsert`, `adminInventoryItemServiceEntriesDelete`, `adminInventoryItemFilesAttach`,
  `adminInventoryItemFilesDelete`, `adminInventoryItemFilesUpsert`. `referenceIds` echoes the row id per input in input order.
  - **Disposal collapsed into `adminInventoryItemsUpsert`.** `AdminInventoryItemInput` carries optional `disposalState` + `disposedAt`;
    there is no `itemDispose` mutation.
  - **Pin toggle collapsed into `adminInventoryItemFilesUpsert`.** `adminInventoryItemFilesUpsert` is edit-only
    (`AdminInventoryItemFileUpsert { itemFileId, label, pinned }`, every input requires `itemFileId` — it never creates rows); the UI
    toggles a pin by passing the existing row's id with the flipped `pinned`. There is no `itemFileTogglePin` mutation. Creating a file join
    is still `adminInventoryItemFilesAttach`.
  - **`adminInventoryItemsReprice` stays separate** because it appends a valuation journal row and updates the cached `currentValueCents` in
    one transaction (a side-effect beyond a field-set).
- Gated once at `Mutation.admin → guardAdminMutation`; child resolvers assume they're guarded (same posture as Media / Projects).

### CQRS wiring

- `src/server/commands/item*.ts` — one file per batch mutation (`adminInventoryItemsUpsert`, `adminInventoryItemsDelete`,
  `adminInventoryItemsReprice`, `adminInventoryItemServiceEntriesUpsert`, `adminInventoryItemServiceEntriesDelete`,
  `adminInventoryItemFilesAttach`, `adminInventoryItemFilesDelete`, `adminInventoryItemFilesUpsert`), each following the
  `adminMediaMoviesUpsert` / `adminTravelTripsUpsert` batch template.
...
- `src/server/mappers/{toGqlAdminInventoryItem,toGqlAdminInventoryItemValuation,toGqlAdminInventoryItemServiceEntry,toGqlAdminInventoryItemFile}.ts`.
