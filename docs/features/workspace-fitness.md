...
| **Strength log + routines/templates** (chosen)                | Five tables: `AdminFitnessExercise`, `AdminFitnessWorkoutRoutine` + `AdminFitnessWorkoutRoutineItem`, `AdminFitnessWorkoutSession` + `AdminFitnessWorkoutSet`. Routines are reusable templates a session can be seeded from. Matches the confirmed v1. |
...
| **Cardio / activity log + fitness goals**                     | Runs/rides with distance/duration, and goals like "bench 100kg". Deferred — the strength log covers the primary use, and folding cardio into `AdminFitnessWorkoutSession` later is additive.                                                           |
| **Sets carry `weight`/`reps` directly, warmup flag** (chosen) | A `AdminFitnessWorkoutSet` is the atomic log unit. `isWarmup` keeps warmups out of PR math. `numeric` weights round-trip half-kg plates. The `(exerciseId)` index backs the "what did I bench last time?" per-exercise read.                           |
...
Five admin-only tables sharing `AdminFitnessMuscleGroup` / `AdminFitnessEquipmentType` enums. `AdminFitnessWorkoutSet` and
`AdminFitnessWorkoutRoutineItem` cascade-delete under their parents and their exercise; `AdminFitnessWorkoutSession.routineId` sets null on
routine delete. Weights are `numeric` (mode `number`). Starting a session from a routine copies its items into sets at creation. The fitness
sub-agent is the primary chat write path; the page is the manual editor and read view.

## Implementation details

### Database schema (`src/server/db/schema.ts`)

`muscleGroups = ['chest','back','legs','shoulders','arms','core','fullBody','cardio','other']` (SDL `AdminFitnessMuscleGroup`);
`equipmentTypes = ['barbell','dumbbell','machine','cable','bodyweight','kettlebell','other']` (SDL `AdminFitnessEquipmentType`).

- **`AdminFitnessExercise`** — `exerciseId`, `name`, `muscleGroup`, `equipment` (nullable), `notes`, timestamps. Index `(muscleGroup)`.
- **`AdminFitnessWorkoutRoutine`** — `routineId`, `name`, `notes`, `position`, timestamps. Index `(position)`.
- **`AdminFitnessWorkoutRoutineItem`** — `routineItemId`, `routineId` FK **CASCADE**, `exerciseId` FK **CASCADE**, `position`, `targetSets`,
  `targetReps`, `targetWeight numeric`, `notes`, timestamps. Index `(routineId, position)`.
- **`AdminFitnessWorkoutSession`** — `sessionId`, `date`, `title`, `routineId` FK **SET NULL**, `durationMinutes`, `notes`, timestamps.
  Index `(date)`.
- **`AdminFitnessWorkoutSet`** — `setId`, `sessionId` FK **CASCADE**, `exerciseId` FK **CASCADE**, `position`, `weight numeric`, `reps`,
  `rpe`, `isWarmup`, `notes`, timestamps. Indexes `(sessionId, position)`, `(exerciseId)`.
...
`adminFitnessExercisesUpsert`/`Delete`, `adminFitnessWorkoutRoutinesUpsert`/`Delete`, `adminFitnessWorkoutRoutineItemsUpsert`/`Delete`,
`adminFitnessWorkoutSessionsUpsert`/`Delete`, `adminFitnessWorkoutSetsUpsert`/`Delete`. All batch, all return `MutationResult`;
`referenceIds` echoes ids in input order — logging a workout is `adminFitnessWorkoutSessionsUpsert` (one session) then
`adminFitnessWorkoutSetsUpsert` (every set) using the session's `referenceIds` as the parent id.
...
normalize in memory. Mappers: `toGqlAdminFitnessExercise`, `toGqlAdminFitnessWorkoutRoutine`, `toGqlAdminFitnessWorkoutRoutineItem`,
`toGqlAdminFitnessWorkoutSession`, `toGqlAdminFitnessWorkoutSet`. Resolver wiring in `resolversCreate.ts`.
...
Session cards use an accordion; "New workout" seeds sets from a chosen routine by firing `adminFitnessWorkoutSessionsUpsert` then
...
- **`agentPersonalAssistantFitness.ts`** — `ToolLoopAgent` factory, model `ADMIN_CHAT_MODEL_FALLBACK_ID`, `stopWhen: [isStepCount(10)]`.
  System prompt: log-a-workout workflow (`adminFitnessWorkoutSessionsUpsert` → `adminFitnessWorkoutSetsUpsert`, "5×5" = five rows),
  auto-create a missing exercise first, answer progression from the snapshot, build-a-routine workflow, and the `needsMoreInfo`/`noOp`
  sentinel contract.
...
- **Tools**: reads `exercisesList`, `routinesList`, `workoutSessionsList`; writes `adminFitnessExercisesUpsert`/`Delete`,
