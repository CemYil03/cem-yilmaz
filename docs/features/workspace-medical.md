...
| **Records + Appointments + Providers**                                                  | Adds a dedicated `MedicalProvider` table (dentist Dr. X with contact info). Sturdier long-term, but Cem doesn't have enough providers for the table to earn its keep — a `providerName varchar` on `AdminMedicalAppointment` covers "which dentist" without a join. Deferred until the provider directory grows enough to want its own page.                                                                                                           |
