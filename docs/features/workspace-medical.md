...
`/workspace/medical` is the admin's private surface for two things: a **health journal** — chat-authored records of symptoms, injuries, and
health notes filed by the medical sub-agent from workspace-assistant conversations — and an **appointment tracker** that groups visits by
...
- [architecture/file-storage.md](../architecture/file-storage.md) — record attachments (rash photos, lab-result screenshots) reuse the
  shared `FileUploads` primitive via a `AdminMedicalRecordFile` join, same wire as inventory receipts.
...
- **Overview** — one card per `AdminMedicalCategory`, always all eight visible so the grid never has holes. Each card shows the category
  label, the last completed visit's date, the next-due date, and a red **Overdue** badge when the next-due is in the past. Cards also
  preview upcoming visits (up to 3) and recent records (up to 3, deep-linkable to the Records tab).
- **Appointments** — list grouped by category. Each row shows title, status chip, scheduled-at, provider, and (when set) `nextDueAt`. Past
  `scheduled` rows get a one-click **Complete** button that calls `adminMedicalAppointmentsUpsert` with a one-element array carrying the
  existing row plus `status: completed` and `completedAt`. The edit dialog opens from the kebab menu — title / category / provider / when /
  next-due / status / notes.
- **Records** — flat list ordered by `occurredAt DESC`. Each card shows title, category chip, severity chip, date, the free-text summary,
  ...
  `adminMedicalRecordsUpsert`; on **edit**, files go straight to `adminMedicalRecordFilesAttach` so the record stays consistent between
  saves.
...
| **Records + Appointments + Providers**                                                  | Adds a dedicated `MedicalProvider` table (dentist Dr. X with contact info). Sturdier long-term, but the admin doesn't have enough providers for the table to earn its keep — a `providerName varchar` on `AdminMedicalAppointment` covers "which dentist" without a join. Deferred until the provider directory grows enough to want its own page.                                                                                                                 |
| **Two tables: `AdminMedicalRecord` + `AdminMedicalAppointment`** (chosen)               | Records are chat-authored health notes with symptoms / body areas / severity. Appointments are scheduled or completed visits with a provider. `AdminMedicalRecord.appointmentId` optionally cross-links the two; on appointment delete the FK is nulled so records survive.                                                                                                                                                                                        |
| **Per-appointment cadence** (`nextDueAt` set manually on each row)                      | No cadence math, no derived overdue flag. Puts the burden on the admin (or the agent) to remember to set the next date after every visit.                                                                                                                                                                                                                                                                                                                          |
...
| **Documentarian + gentle triage** (chosen)                                              | The sub-agent captures what the admin tells it into a structured record and offers low-risk practical suggestions ("if it doesn't improve in two weeks, worth showing a dermatologist"), but does NOT diagnose or prescribe. Its system prompt carries an explicit **red-flag refusal list** (chest pain, stroke signs, breathing difficulty, anaphylaxis, suicidal ideation) — on trigger it refuses to file a record and tells the admin to seek emergency care. |
...
| **Attachments piggyback on `FileUploads` via a `AdminMedicalRecordFile` join** (chosen) | Same wire as inventory receipts. When the admin drops a photo into the workspace chat, it already persists as a `FileUpload` and the orchestrator sees it as a `FilePart` — the medical delegate just forwards the `fileUploadIds` so the sub-agent can attach them to the record it files, atomically, via `adminMedicalRecordsUpsert.fileUploadIds`.                                                                                                             |
...
Two dedicated tables — `AdminMedicalAppointment` and `AdminMedicalRecord` — plus a `AdminMedicalRecordFile` join into the shared
`FileUploads`. Both core tables carry a `AdminMedicalCategory` enum used for grouping and cadence. Records optionally cross-link to
appointments. Attachments live on records only in v1 (a symmetrical `MedicalAppointmentFiles` join is a follow-up if it's ever needed for
lab-result PDFs on a visit).
...
editing is available on the workspace page for anything the sub-agent didn't file (or that the admin wants to backdate).
...
- `topics text[]` — free-form cluster tags; same shape as `AdminMediaMovie.topics`
- `createdAt`, `updatedAt`
...
- `appointmentId uuid` FK → `AdminMedicalAppointment.appointmentId`, **`ON DELETE SET NULL`** — records survive appointment cleanup
...
- `recordId uuid` FK → `AdminMedicalRecord.recordId`, **`ON DELETE CASCADE`**
- `fileUploadId uuid` FK → `FileUploads.fileUploadId`, **`ON DELETE CASCADE`** (same posture as `AdminInventoryItemFile`)
...
- `adminMedicalAppointmentFindMany: [AdminMedicalAppointment!]!` — every appointment; ordered `scheduled` first (soonest first), then
  everything else by `scheduledAt DESC`
- `adminMedicalRecordFindMany: [AdminMedicalRecord!]!` — every record; ordered by `occurredAt DESC` (falling back to `createdAt`), files
  pre-joined
- `adminMedicalCategoryOverviewFindMany: [AdminMedicalCategoryOverview!]!` — one row per `AdminMedicalCategory` with `defaultCadenceMonths`,
  `lastCompletedAt`, `nextDueAt`, `isOverdue`, `upcoming: [AdminMedicalAppointment!]!`, `recentRecords: [AdminMedicalRecord!]!`. Cadence
  math lives in `adminMedicalCategoryOverviewFindMany.ts`; categories with no data still surface so the overview grid never has holes.
...
- `adminMedicalAppointmentsUpsert(medicalAppointments: [AdminMedicalAppointmentInput!]!): MutationResult!` — batch create-or-edit. Each row
  with an `appointmentId` is updated; each without one is inserted. **Completing an appointment** is a one-element upsert carrying the
  existing row (from the subscription payload) plus `status: completed` and `completedAt` set (optionally `nextDueAt`) — the UI "Mark
  completed" button and the sub-agent both do this. There is no separate `medicalAppointmentComplete` mutation.
- `adminMedicalAppointmentsDelete(appointmentIds: [ID!]!): MutationResult!` — linked `AdminMedicalRecord.appointmentId` are nulled via the
  FK
- `adminMedicalRecordsUpsert(medicalRecords: [AdminMedicalRecordInput!]!): MutationResult!` — the primary write. Each row accepts optional
...
- `adminMedicalRecordsDelete(recordIds: [ID!]!): MutationResult!` — cascade removes join rows; `FileUploads` bytes stay put
- `adminMedicalRecordFilesAttach(inputs: [AdminMedicalRecordFileAttachInput!]!): MutationResult!` — attach files to existing records; each
...
- **`agentPersonalAssistantMedical.ts`** — `ToolLoopAgent` factory. System prompt is a large block that: (a) pins the role to
  ...
  a record and instead direct the admin to emergency care, (d) explains each of the nine tools, (e) embeds a live medical snapshot.
...
- **`toolDelegateToMedical.ts`** — orchestrator-side delegate. Persists the parent `chatMessagesToolCall` row, closes over a
  ...
  brief so it can pass them straight through on `adminMedicalRecordsUpsert.fileUploadIds`.
...
The end-to-end flow when the admin pastes a photo of a rash into the workspace chat and asks about it:
...
5. **Sub-agent**: asks clarifying questions if needed (via the `needsMoreInfo` sentinel), then calls `adminMedicalRecordsUpsert` with a
   one-element array carrying `fileUploadIds: [<id>]`. The command validates ownership, inserts the record, and inserts the
   `AdminMedicalRecordFile` join row atomically.
...
- **Overdue-reminder job** — a `pg-boss` job that fires on overdue categories and pings the assistant to nudge the admin.
